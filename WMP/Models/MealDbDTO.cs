...
{
    [JsonPropertyName("idMeal")] public string Id { get; set; }
    [JsonPropertyName("strMeal")] public string Name { get; set; }
    [JsonPropertyName("strMealAlternate")] public string? AlternativeMeal { get; set; }
    [JsonPropertyName("strCategory")] public string Category { get; set; }
    [JsonPropertyName("strArea")] public string? Area { get; set; }
    [JsonPropertyName("strInstructions")] public string Instructions { get; set; }
    [JsonPropertyName("strMealThumb")] public string? MealThumbnail { get; set; }
    ...
    [JsonPropertyName("strSource")] public string? Source { get; set; }
    [JsonPropertyName("strImageSource")] public string? SourceImage { get; set; }
    [JsonPropertyName("strCreativeCommonsConfirmed")] public string? CreativeCommonsConfirmed { get; set; }
    [JsonPropertyName("dateModified")] public string? DateModified { get; set; } 
    public List<MealDbIngredient> Ingredients => GetIngredientList();

    private List<MealDbIngredient> GetIngredientList()
    {
        var result = new List<MealDbIngredient>();

        for (int i = 1; i <= 20; i++)
        {
            string ingredient = (string)this.GetType().GetProperty($"Ingredient{i}")?.GetValue(this);
            string measure = (string)this.GetType().GetProperty($"Measure{i}")?.GetValue(this);

            if (!string.IsNullOrWhiteSpace(ingredient))
            {
                result.Add(new MealDbIngredient()
                {
